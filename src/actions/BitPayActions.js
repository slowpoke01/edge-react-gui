...
async function fetchBitPayJsonResponse(uri: string, init: Object): Promise<Response> {
  ...
  if (!fetchResponse.ok || fetchResponse.status !== 200) {
    ...
    throw new BitPayError(BitPayErrorCode.FetchFailed, { uri, body, header, method, statusCode, text })
  }
...
export async function launchBitPay(dispatch: Dispatch, uri: string, currencyWallets: { [walletId: string]: EdgeCurrencyWallet }): Promise<void> {
  ...
  if (invoiceResponse.instructions.length > 1) {
    throw new BitPayError(BitPayErrorCode.MultiInstructionInvoice, { uri })
  }
  ...
  if (invoiceInstruction.outputs) {
    if (invoiceInstruction.outputs.length > 1) {
      throw new BitPayError(BitPayErrorCode.MultiOutputInvoice, { uri })
    }
  } else throw new BitPayError(BitPayErrorCode.EmptyOutputInvoice, { uri })
  ...
  const unsignedHex = unsignedTx.otherParams ? unsignedTx.otherParams.txJson.hex : ''
  const signedTx = await wallet.signTx(unsignedTx)
  const signedHex = signedTx.otherParams ? signedTx.otherParams.txJson.hex : ''
  if (unsignedHex === '' || signedHex === '') throw new BitPayError(BitPayErrorCode.EmptyVerificationHexReq, { uri })
  ...
  if (verificationPaymentResponse.transactions.length !== 1 || unsignedHex !== verificationPaymentResponse.transactions[0].tx)
    throw new BitPayError(BitPayErrorCode.TxVerificationMismatch, { uri })
