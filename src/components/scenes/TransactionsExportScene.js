...
class TransactionsExportSceneComponent extends React.PureComponent<Props, State> {
  ...
  handleSubmit = async (): Promise<void> => {
    ...
    const csvFile = await exportTransactions('csv', sourceWallet, txs, transactionOptions)
    ...
    if (isExportQbo) {
      const qboFile = await exportTransactions('qbo', sourceWallet, txs, transactionOptions)
      files.push({
        contents: qboFile,
